...
contract PricingModule {
    ...
    function calculatePerpFee(
        ...
        if(_isMarginZero) {
            value = calculatePerpValue(_price, 1e12 * _lowerSqrtPrice / (2**96), 1e12 * _upperSqrtPrice / (2**96), _volatility);
        } else {
            value = calculatePerpValue(_price, (2**96) * 1e12 / _upperSqrtPrice, (2**96) * 1e12 / _lowerSqrtPrice, _volatility);
        }
 
        return convertSizeToLiq(
            _isMarginZero,
            _lowerSqrtPrice,
            _upperSqrtPrice,
            1e18,
    ...
    function calculatePerpValue(
        ...
        uint256 r2 = calculateR2(
            calculateT(r, _volatility) + ONE, _volatility
        );
    ...
    function convertSizeToLiq(bool _isMarginZero, uint160 lowerSqrtPrice, uint160 upperSqrtPrice, uint256 _amount, uint256 _value) public view returns (uint256) {
        uint160 liquidity;
        if(_isMarginZero) {
