...
library VaultLib {
    ...
    function getCollateralPositionAmounts(
        ...
        BaseToken.TokenState memory _tokenState1,
        uint160 _sqrtPrice
    ) external view returns (uint256 totalAmount0, uint256 totalAmount1) {
    ...
    ) internal pure returns (uint256 totalAmount0, uint256 totalAmount1) {
        for (uint256 i = 0; i < vault.lpts.length; i++) {
            if (vault.lpts[i].isCollateral && vault.lpts[i].rangeId == _rangeId) {
                totalAmount0 = PredyMath.mulDiv(
                    (_perpState.fee0Growth.sub(vault.lpts[i].fee0Last)),
                    vault.lpts[i].liquidityAmount,
                    FixedPoint128.Q128
                );
                totalAmount1 = PredyMath.mulDiv(
                    (_perpState.fee1Growth.sub(vault.lpts[i].fee1Last)),
                    vault.lpts[i].liquidityAmount,
                    FixedPoint128.Q128
                );
            }
    ...
    function getEarnedDailyPremium(
        ...
        bytes32 _rangeId,
        DataType.PerpStatus memory _perpState
    ) external pure returns (uint256 marginValue) {
        for (uint256 i = 0; i < _vault.lpts.length; i++) {
            ...
            if (_vault.lpts[i].isCollateral && rangeId == _rangeId) {
                marginValue = marginValue.add(
                    PredyMath.mulDiv(
                        (_perpState.premiumGrowthForLender.sub(_vault.lpts[i].premiumGrowthLast)),
    ...
    function getPaidDailyPremium(
        ...
        for (uint256 i = 0; i < _vault.lpts.length; i++) {
            ...
            if (!_vault.lpts[i].isCollateral && rangeId == _rangeId) {
                marginValue = marginValue.add(
                    PredyMath.mulDiv(
                        (_perpState.premiumGrowthForBorrower.sub(_vault.lpts[i].premiumGrowthLast)),
                        _vault.lpts[i].liquidityAmount,
                        1e18
                    )
                );
